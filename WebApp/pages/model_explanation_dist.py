...
class CustomClassificationTab(ExplainerComponent):
    def __init__(self, explainer, name = None):
        ...
        self.contributions = ClassificationComponent(explainer, 
                                                     hide_cutoff = False, hide_selector = True, 
...
class CustomFeatureImportanceTab(ExplainerComponent):
    ...
    def make_fi_graph(self):
        pkl_fi_filename = "resources/distance/distance_feature_importance.pkl"
...
class CustomFeatureDependenceTab(ExplainerComponent):
    def __init__(self, explainer, name = None):
        ...
        self.shap_dependence = ShapDependenceComponent(explainer,
                                ...
                                col = 'State', color_col = "Pressure(in)"
                                )
                    ...
                    dbc.Card([
                        dbc.CardHeader([
                            html.Div([
                                ...
                                html.H6(f"Correlation between feature and the impact to the Traffic Accident Distance", className = "card-subtitle")
                            ]),
                        ]),
                        dbc.CardBody(
                            html.Div([
                                html.Label('Feature:', className = 'form-label'),
                                dcc.Dropdown(feature_list, 'Pressure(in)', id = 'dist-feature-dropdown'),
                                dcc.Loading(
    ...
    @app.callback(
        Output('dist-pdp-output', 'children'),
...
class CustomIndividualPredictionTab(ExplainerComponent):
    def __init__(self, explainer, name = "DistIndividualPrediction"):
        super().__init__(explainer, title = "Individual Predictions")
        
        self.index = ClassifierRandomIndexComponent(explainer, name = "DistClassRandIndex",
                                                    subtitle = "Select which record to view its details",
                                                    hide_selector = True, index = 1,
                                                    hide_pred_or_perc = True, hide_slider = True
                                                    )
    ...
    def layout(self):
        dataframe = self.explainer.get_contrib_summary_df(str(1), topx = 26, sort = "abs")
...
###
# Main Functions for Distance - Explainability
###
@app.callback(Output('dist-tab-output', 'children'),
              Input('dist-tabs-options', 'value'))
...
def build_classification_graph():
    ...
    basePath = res_dist_dir + 'model_data/'
...
def build_accuracy_bar_graph(algo_list, accuracy_list):
    ...
    fig.update_layout(yaxis_range=[80, 82], plot_bgcolor='white')
    fig.update_traces(textfont_size = 14, textangle = 0, textposition = "outside")
    new = {'LR': 'Logistic Regression', 'RF': 'Random Forest', 'KNN': 'K-Nearest Neighbour', 'DT': 'Decision Trees'}
    fig.for_each_trace(lambda t: t.update(name = new[t.name]))
    fig.update_yaxes(
...
def build_error_bar_graph(algo_list, error_list):
    ...
    fig.update_layout(yaxis_range=[82, 105], plot_bgcolor='white')
    fig.update_traces(textfont_size = 14, textangle = 0, textposition = "outside")
    new = {'LR': 'Logistic Regression', 'RF': 'Random Forest', 'KNN': 'K-Nearest Neighbour', 'DT': 'Decision Trees'}
    fig.for_each_trace(lambda t: t.update(name = new[t.name]))
    fig.update_yaxes(
...
pkl_X_res_filename = res_dist_dir + "X_res.pkl"
pkl_y_res_filename = res_dist_dir + "y_res.pkl"
...
def get_pdp_values(feature):
    ...
    pred_avg_dist_delta_df = pd.DataFrame()

    distance_pkl_model_filename = res_dist_dir + "distance_rfc_best_model.pkl"
    ...
    list_of_feature_values, pred_avg_dist_delta_ls = [], []

    Q3 = np.quantile(X_copy[feature], 0.75)
    Q1 = np.quantile(X_copy[feature], 0.25)
    print('Feature: ', feature, ', Q1: ', Q1, ', Q3:', Q3)

    dist_range_delta = np.sum(y_copy.unique()) / y_copy.nunique()
    print('Distance Delta: ', dist_range_delta)

    # Iterate through values in selected feature and predict its Distance
    for i in list_of_values: 
        if i >= Q1 and i <= Q3:
            X_copy[feature] = i
            y_pred = model.predict(X_copy)
            
            # Predicted Distance (Average)
            y_pred_avg = sum(y_pred) / len(y_pred)
            dist_delta = y_pred_avg - dist_range_delta
            pred_avg_dist_delta_ls.append(dist_delta)

            list_of_feature_values.append(i)

    pred_avg_dist_delta_df['Distance'] = pred_avg_dist_delta_ls
    pred_avg_dist_delta_df[feature] = list_of_feature_values

    return pred_avg_dist_delta_df

def build_pdp_graph(feature_delta, feature_name):
    pdp_fig = px.line(feature_delta, x = feature_name, y = 'Distance',
                        labels = {
                            "Distance": "Distance Impact",
                        }
                    )
...
def build_contribution_graph(dataframe):
    ...
    fig = px.bar(contrib_df.head(10), x = "Reason", y = "Effect",
        color = contrib_df.head(10).loc[:, 'Effect'] > 0,
...
@app.callback(
    Output("dist-contrib_card", "children"),
    Input("random-index-clas-index-DistClassRandIndex", "value")
)
def get_updated_index(index):
    return_array = []

    if index is not None and index != "":
        contrib_df = classifier_explainer.get_contrib_summary_df(str(index), topx = 26, sort="abs")
...
classifier_explainer = decompress_pickle('resources/distance/distance_model_explainer_records.pbz2') 
# classifier_explainer = ClassifierExplainer.from_file('resources/distance/distance_model_explainer.pbz2')  
...
layout = html.Div([
    html.Br(),
    html.Div([
        html.H1(children = 'Model Explainer: Distance', 
            style = {'margin-left':'10px', 'margin-bottom':'1.5rem'})
    ], style = {"margin-left": "auto", "margin-right": "auto", "width": "90%"}),
    html.Div([
        dcc.Tabs(id = "dist-tabs-options", value = 'classification-stats', children = [
